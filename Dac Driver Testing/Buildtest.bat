...
IF EXIST "VF2Driver.p" del "VF2Driver.p"
IF EXIST "VF2Driver.p" goto LABLERROR1
...
IF "%1"=="-logerrors" ( "AS/Win32/asw.exe" -xx -q -c -E -L -A  "VF2Driver.asm" ) ELSE "AS/Win32/asw.exe" -xx -q -c -L -A "VF2Driver.asm"

REM // if there were errors, a log file is produced
IF "%1"=="-logerrors" ( IF EXIST "VF2Driver.log" goto LABLERROR3 )

REM // combine the assembler output into a rom
IF EXIST "VF2Driver.p" "AS/Win32/s3p2bin" "VF2Driver.p" "z80.bin"

REM // done -- pause if we seem to have failed, then exit
IF NOT EXIST "VF2Driver.p" goto LABLPAUSE
...
:LABLERROR1
echo Failed to build because write access to "VF2Driver.p" was denied.
pause
...
echo ***************************************************************************
echo *                                                                         *
echo *   There were build errors/warnings. See "VF2Driver.log" for more details.   *
echo *                                                                         *
echo ***************************************************************************
echo.
pause
